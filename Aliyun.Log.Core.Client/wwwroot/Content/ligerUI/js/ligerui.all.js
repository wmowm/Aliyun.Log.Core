    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _init: function ()
        {
            ...
            if (p.tree)//启用分页模式
            {
                ...
                p.tree.isParent = p.tree.isParent || function (rowData)
                {
                    ...
                    if (rowData[p.tree.childrenName] == null || !rowData[p.tree.childrenName] || rowData[p.tree.childrenName].length == 0)
                        return false;
                    return true;
                };
        ...
        */
        ...
        _initColumns: function ()
        {
            ...
            var totalWidth = $(parent.document).width() - 230;
            var lastColNum = 0;
            ...
            if (lastColNum > 0) {
                var colWidth = parseInt(totalWidth / lastColNum);
                var num = lastColNum;
                $(g.columns).each(function (i, column) {
                    if (column._width < 20) {
                        lastColNum--;
                        if (lastColNum == 0) {
                            column._width = totalWidth - colWidth * (num - 1);
                        } else {
        ...
        {
            var g = this, p = this.options;
            if (column._width && column._width > 20) return column._width;
        ...
        _f_gridLayout_autoAdapt: function (ChangedWidth) {
            ...
            for (var level = 1; level <= g._columnMaxLevel; level++) { 
                ...
                $(columns).each(function (i, column) {
                    if (column.isrownumber || column.hide || column.ischeckbox || column._hide) {//扣除序号列和隐藏列  
                        colNum = colNum - 1;
                    }
                });
                var columnWidth = parseInt((ChangedWidth - 12) / colNum);//计算修改后的列宽度  
                ...
                if (g.gridview2 != undefined) {
                    ...
                    if (gridHeaderInner != undefined) {
                        jQuery("." + gridHeaderInner.className).css({ width: ChangedWidth - 12 });
                    }
                }
                var gridBodyInner = g.gridbody[0].children[0];
                if (gridBodyInner != undefined) {
                    var str = "." + gridBodyInner.className;
                    jQuery(str).css({ width: ChangedWidth - 12 });//修改l-grid-body-inner的宽度  
